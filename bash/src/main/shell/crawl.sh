...
function __bin_nutch {
    # run $bin/nutch.sh, exit if exit value indicates error

    echo "$bin/nutch.sh $@" ;# echo command and arguments
    "$bin/nutch.sh" "$@"
...
do
  ...
  echo "$bin/nutch.sh generate ${generate_args[@]}"
...
# on a per segment basis
  ...
  #__bin_nutch org.apache.nutch.sh.scoring.webgraph.Loops $commonOptions -webgraphdb "$CRAWL_PATH"
